...
def main(data, dataset_name, n_splits=30, test_size=0.1, dsel_size=0.33, ensemble_size=40):
    """
    Parameters:
    ...
    for train_index, test_index in stf_split.split(X, y):
        ...
        # 创建分类器池
        pool = create_classifier_pool(ensemble_size=ensemble_size)

        # 训练分类器池中的每个基分类器
        pool.fit(X_train_fit, y_train_fit)

        # 初始化动态选择模型
        models = get_models(pool)
...
# 创建分类器池的函数
def create_classifier_pool(ensemble_size=40):
    """
    创建包含多种分类器的池。
    ...
    pool = BaggingClassifier(
        base_estimator=DecisionTreeClassifier(criterion='entropy', min_samples_split=10, min_samples_leaf=2,
                                              random_state=10),
        n_estimators=ensemble_size, bootstrap=True, random_state=10
    )
    return pool
...
def get_models(pool):
    ...
    models = [
        # DES models
        DESFH(pool_classifiers=pool, k=7, theta=0.1, mu=0.9, random_state=0),
        KNORAE(pool_classifiers=pool, k=7, random_state=0),
        KNORAU(pool_classifiers=pool, k=7, random_state=0),
        DESP(pool_classifiers=pool, k=7, random_state=0),
        DESKL(pool_classifiers=pool, k=7, random_state=0),
        KNORAE(pool_classifiers=pool, k=7, random_state=0),
        KNORAU(pool_classifiers=pool, k=7, random_state=0),
        KNOP(pool_classifiers=pool, k=7, random_state=0),
        METADES(pool_classifiers=pool, k=7, Kp=5, Hc=0.7),

        # Group-based DES models.
        DESKNN(pool_classifiers=pool, k=7, metric='DF', random_state=0),
        DESKNN(pool_classifiers=pool, k=7, metric='Q', random_state=0),
        DESKNN(pool_classifiers=pool, k=7, metric='ratio', random_state=0),

        # Classic Dynamic Classifier Selection (DCS) models.
        OLA(pool_classifiers=pool, k=7, random_state=0),
        LCA(pool_classifiers=pool, k=7, random_state=0),
        MLA(pool_classifiers=pool, k=7, random_state=0),
        MCB(pool_classifiers=pool, k=7, random_state=0),

        # Static Selection (SS) models
        SingleBest(pool_classifiers=pool, random_state=0),
        StaticSelection(pool_classifiers=pool, random_state=0),

        # The proposed DES-AS using Truncated Monte Carlo simulation
